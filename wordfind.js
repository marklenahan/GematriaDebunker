...
if (args.length === 0) {
  console.error('Usage: wordfind <word> [...] [-word ...]');
  process.exit(1);
}

const plusWords = args.filter(a => !a.startsWith('-'));
const minusWords = args.filter(a => a.startsWith('-')).map(a => a.slice(1));
...
const inputTotal = plusTotal - minusTotal;
if (minusWords.length > 0) console.log(`\nnet total: ${inputTotal}`);
console.log();
